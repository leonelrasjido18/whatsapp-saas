...
import { applyMessageStatusUpdate } from "@/features/inbox/services/status-updates";
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const { contact, conversation, message } = await processInbound(
      ...
      {
        channel: "whatsapp",
        identity: { kind: "phone", phone: normalized.from },
        profileName: normalized.customerName,
        type: normalized.type,
        text: normalized.text,
        providerMessageId: normalized.wamid,
      },
    );
